...
#define WS_RAM_GPU VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
#define WS_RAM_CPU VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT 
...
namespace WS {
    ...
    void destroyObjects(VkDescriptorSetLayout descriptorSet, VkDevice device) {
    ...
    VkDescriptorSetLayout createDescriptorSetLayout(VkDevice device) {
        ...
        if (uboResult != VK_SUCCESS) {
            logError(uboResult,"Descriptor Set Layout: ");
            throw std::runtime_error("Descriptor Set Layout Failed Creation");
        }
        return descriptorSetLayout;
    };
    std::tuple<std::vector<VkBuffer>, std::vector<VkDeviceMemory>> createUniformBuffers(VkDevice device, VkPhysicalDevice physicalDevice, std::vector<VkImage> swapchainImages) {
        VkDeviceSize size = sizeof(uniformBufferObject);
        std::vector<VkBuffer> UniformBuffers; std::vector<VkDeviceMemory> UniformMemory;

        for (int i = 0; i < swapchainImages.size(); i++) {
            createBuffer(device, physicalDevice, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, UniformMemory[i], size);
        }
        return {UniformBuffers, UniformMemory};
    }
    VkPipeline createGraphicsPipeline(VkDevice device,VkShaderModule vertexShader, VkShaderModule fragmentShader, VkRenderPass renderPass, VkExtent2D swapchainExtent,VkPhysicalDevice physicalDevice, VkDescriptorSetLayout descriptorSet, VkPolygonMode DrawMode = VK_POLYGON_MODE_FILL) {
    ...
    VkBuffer vertexBuffer = VK_NULL_HANDLE, VkBuffer indexBuffer = VK_NULL_HANDLE, std::vector<uint16_t> indices = {}) {
    ...
    std::vector<VkFramebuffer>& framebuffers, VkCommandPool commandPool, std::vector<VkCommandBuffer>& commandBuffers, VkPipeline& graphicsPipeline, VkRenderPass& renderPass, VkSwapchainKHR& swapchain, std::vector<VkImageView>& imageViews, VkDescriptorSetLayout descriptorSet,
    std::tuple<VkBuffer, VkBuffer, std::vector<uint16_t>> Buffers, VkPolygonMode DrawMode = VK_POLYGON_MODE_FILL) {
        ...
        graphicsPipeline = createGraphicsPipeline(device, vertexShader, fragmentShader, renderPass, extent, physicalDevice, descriptorSet, DrawMode);
        framebuffers = createFramebuffers(imageViews, renderPass, extent, device);
        commandBuffers = createCommandBuffers(device, commandPool, framebuffers, renderPass, graphicsPipeline, extent, std::get<0>(Buffers), std::get<1>(Buffers), std::get<2>(Buffers));
    ...
    VkBuffer createBuffer(VkDevice device, VkPhysicalDevice physicalDevice, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkDeviceMemory& bufferMemory, VkDeviceSize size) {
        VkBuffer Buffer;
    ...
    void allocateBuffer(VkDevice device, VkDeviceMemory& deviceMemory, const void* copy, VkDeviceSize size) {
        void* data;
        vkMapMemory(device, deviceMemory, 0, size, 0, &data);
        memcpy(data, copy, size_t(size));
    ...
    void copyBuffer(VkDevice device, VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size, VkCommandPool commandPool, VkQueue queue) {
        ...
        if (allocResult != VK_SUCCESS) {
            logError(allocResult, "Command Buffer Allocation: ");
    ...
    struct Devices {
        VkDevice Device;
        VkPhysicalDevice PhysicalDevice;
        std::vector<WS::Queue> QueueList;
    };
    WS::Devices createDevices(VkInstance instance) {
        WS::Devices Devices;
        auto PhysicalDevice = findPhysicalDevice(instance);
        auto QueueList = findQueues(PhysicalDevice);
        auto Device = createDevice(PhysicalDevice,QueueList);
        for (auto i : QueueList) {
            vkGetDeviceQueue(Device, i.Index, 0, &i.Queue);
        }
        Devices.Device = Device;
        Devices.PhysicalDevice = PhysicalDevice;
        Devices.QueueList = QueueList;
        return Devices;
    }
    ...
    WS::Presentation createPresentationObjects(VkInstance instance, const char* title, int width, int height) {
        WS::Presentation Presentation;
        auto Window = createWindow(title,width, height, instance);
        Presentation.Window = std::get<0>(Window);
        Presentation.Surface = std::get<1>(Window);
        return Presentation;
    }
    ...
    struct Graphics {
        ...
        VkPolygonMode DrawModeState = VK_POLYGON_MODE_FILL;
        std::vector<uint16_t> Indices { 0, 1, 2, 2, 3, 0 };
    };
    WS::Graphics createGraphics(Devices devices, Presentation presentation, const char* vertexPath, const char* fragmentPath) {
        WS::Graphics Graphics;
        auto Swapchain = createSwapchain(devices.PhysicalDevice, devices.Device, presentation.Surface, presentation.Window, devices.QueueList, Graphics.format, Graphics.extent, Graphics.graphicsQueueIndex, Graphics.transferQueueIndex);
        auto SwapchainImages = getSwapchainImages(devices.Device, Swapchain);
        auto SwapchainImageViews = createImageViews(SwapchainImages, devices.Device, Graphics.format);
        
        auto RenderPass = createRenderpass(devices.Device, Graphics.format);
        auto DescriptorSet = createDescriptorSetLayout(devices.Device);
        auto vertexShader = createShaderModule(vertexPath, devices.Device);
        auto fragmentShader = createShaderModule(fragmentPath, devices.Device);
        auto GraphicsPipeline = createGraphicsPipeline(devices.Device, vertexShader, fragmentShader, RenderPass, Graphics.extent, devices.PhysicalDevice, DescriptorSet, Graphics.DrawModeState);
        auto Framebuffers = createFramebuffers(SwapchainImageViews, RenderPass, Graphics.extent, devices.Device);
        auto CommandPool = createCommandPool(devices.Device, Graphics.graphicsQueueIndex);
        auto CommandBuffers = createCommandBuffers(devices.Device, CommandPool, Framebuffers, RenderPass, GraphicsPipeline, Graphics.extent /*Vertex Buffer, Index Buffer, indices */);
        

        Graphics.Swapchain = Swapchain;
        Graphics.SwapchainImages = SwapchainImages;
        Graphics.SwapchainImageViews = SwapchainImageViews;
        ...
        Graphics.Commandbuffers = CommandBuffers;
...
std::vector<vertex> pointsToRect(rect r) {
    std::vector<vertex> vertices {
        {{r.x,r.y},              {0.0f,0.2f,1.0f}},
        {{r.x + r.w,r.y},        {1.0f,1.0f,1.0f}},
        {{r.x + r.w, r.y + r.h}, {0.0f,0.2f,1.0f}},
        {{r.x, r.y + r.h},       {0.0f,0.2f,1.0f}}
    };
    return vertices;
}
...
int main() {
    ...
    rect r {-0.9f,-0.9f,1.0f,1.0f};
    const auto vertices = pointsToRect(r);
    ...
    auto IndexStagingBuffer = WS::createBuffer(Device, PhysicalDevice, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, IndexStagingBufferMemory, sizeof(indices[0]) * indices.size());
    WS::allocateBuffer(Device, IndexStagingBufferMemory, indices.data(), sizeof(indices[0]) * indices.size());
    auto IndexBuffer = WS::createBuffer(Device, PhysicalDevice, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, IndexBufferMemory, sizeof(indices[0]) * indices.size());
    

// make function for recreation in swapchain
    ...
    for (int i = 0; i < SwapchainImages.size(); i++) {
        UniformBuffers[i] = WS::createBuffer(Device, PhysicalDevice, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, UniformBufferMemory[i], UniformBufferSize);
    }
//
    ...
    auto DrawMode = VK_POLYGON_MODE_FILL;
    auto DescriptorSet = WS::createDescriptorSetLayout(Device);
    auto GraphicsPipeline = WS::createGraphicsPipeline(Device, VertexShader, FragmentShader, RenderPass, imageExtent, PhysicalDevice, DescriptorSet, DrawMode);
    ...
    auto CommandBuffers = WS::createCommandBuffers(Device, CommandPool, FrameBuffers, RenderPass, GraphicsPipeline, imageExtent, VertexBuffer, IndexBuffer, indices);
 

    WS::copyBuffer(Device, IndexStagingBuffer, IndexBuffer, sizeof(indices[0]) * indices.size(), CommandPool, GraphicsQueue);
    WS::destroyObjects(IndexStagingBuffer, IndexStagingBufferMemory, Device);

    WS::copyBuffer(Device, StagingBuffer, VertexBuffer, sizeof(vertices[0]) * vertices.size(), CommandPool, GraphicsQueue);
    WS::destroyObjects(StagingBuffer, StagingBufferMemory, Device);
    ...
    int frames = 0;
    int startRenderTime = 0;
    int globalClock = clock();
    while(!glfwWindowShouldClose(Window)) { 
        if (clock()-globalClock < 1500) {
        ...
        if (imageResult == VK_ERROR_OUT_OF_DATE_KHR) {
            ...
            queueList, FrameBuffers, CommandPool, CommandBuffers, GraphicsPipeline, RenderPass, Swapchain, SwapchainImageViews, DescriptorSet, {VertexBuffer,IndexBuffer,indices}, DrawMode);
        }
        ...
        frames++;
        if (clock()-startRenderTime >= 1000 && clock()-globalClock > 3000) {
    ...
    WS::destroyObjects(DescriptorSet, Device);
    WS::destroyObjects(IndexBuffer, IndexBufferMemory, Device);
